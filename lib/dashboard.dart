...
// ─── Entry point (uncomment when using standalone) ───────────────────────────
// void main() {
//   runApp(const DistractApp());
// }
...
class _DashboardScreenState extends State<DashboardScreen>
  ...
  void initState() {
    ...
    _platform.setMethodCallHandler((call) async {
      if (call.method == 'onScrollUpdated') {
        setState(() => _reelsCount = call.arguments as int);
      }
    });
  }
  ...
  Future<void> _initializeData() async {
    try {
      final bool acc = await _platform.invokeMethod('isAccessibilityEnabled');
      final bool overlay = await _platform.invokeMethod('isOverlayEnabled');
      final int count = await _platform.invokeMethod('getInitialCount');
      setState(() {
